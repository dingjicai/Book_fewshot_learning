...
def preprocess_train(x, y):
    ...
    #加噪音
    x = x + 0.5 * tf.convert_to_tensor(np.random.normal(loc=0.0, scale=1.0, size=x.shape), np.float32)
...
def LeNet():
    ...
    reshape = layers.Reshape((1, 1, 120))(flatten)
...
plt.show()
#提取推理数据前1000张图片的编码向量（长度120）
from tensorflow.keras import backend as K

layer_sub = K.function([model.layers[0].input], [model.get_layer(name='flatten').output])
vec_en = np.zeros((300, 120), dtype=float)
...
pca = pca.fit(vec_en)
vec_en_pca = pca.transform(vec_en)
